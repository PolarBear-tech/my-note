...
- `deque` → 高效队列 / 滑动窗口
- `namedtuple` → 轻量级数据模型
- `OrderedDict` → 有序字典，缓存实现

一句话： `collections` 就是 Python 内置数据结构的“超级加强版”，能让你的代码 **更简洁、更高效、更 Pythonic** 。

## 2\. functools：函数式编程工具

在 Python 里，函数是 **一等公民** ：可以作为参数传递、作为返回值输出，还能动态生成新的函数。
...
小技巧： `partial` 常常用在 **并发库 concurrent.futures** 或 **GUI 事件绑定** 中。
...
虽然 Python 更推荐直接用 `sum(nums)` 、 `min(nums)` 等内置函数，
...
它提供了很多 **惰性计算** （lazy evaluation）的迭代器：不会一次性生成完整结果，而是按需计算，既省内存，又高效。
...
排列和组合是算法常见场景， `itertools` 提供了现成函数。
...
- `islice(iterable, start, stop, step)` → 类似切片，但对迭代器有效
- `zip_longest(a, b, fillvalue=None)` → 对齐两个序列，缺失用 `fillvalue` 填补
- `product([1,2], ["A","B"])` → 笛卡尔积，常用于参数组合
```python
...
4. **`itertools`** → 可以进一步用于合并多份日志文件，或做复杂组合过滤。
...
✨ 虽然它们只是 **“标准库里的小工具”** ，但却能让 Python 在 **数据处理 / 脚本开发 / 算法实现** 中更简洁、更高效、更优雅。
